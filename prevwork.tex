...
Additionally, non symmetric ones normally imply movement constrains to the other parts.
...
However, parameters for symmetry detection and shape deformation still have to be manually given to the algorithm.
...
Nevertheless, their method is constrained to only rigid transformations.
...
Which iteratively fits envelopes to the surface and does a deforming simplification to smooth out details.
Other methods has been explored as well, such as planar based ones \cite{McCrae2011}.
However, their approach is limited to a learning phase with a training data set manually marked.
\cite{Yumer2012} proposed a controllable abstraction approach uses a combination of RANSAC and volume-centric primitive fitting to produce a range of simplified models.
The models were approximated by fitting a set of basic primitives to the model; which are then deformed with a polynomial surface. 
An alternative to \cite{Attene2006} segmentation is also presented, addressing the deficiencies in previous research.
