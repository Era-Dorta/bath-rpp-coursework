...
Either generating arrows indicating the overall fluid movement, displaying an animation or showing illustrative key frames.
...
Therefore, techniques to embed motion in a single picture will be used.
...
Therefore, a geometric analysis can be performed and the needed information can be extracted from models that do not have extra time information embedded.
Which is the case for most of the models available in the model libraries mentioned in Section \ref{sec:partAnalysis}.
...
Since in hydraulic equipment there are not complex fluid flow situations, there is not clear advantage in applying surface based integral objects.
...
One example could be an evaluation on how well the test subjects understood the machinery internal workings with a series of questions, estimating their knowledge on the equipment before examining the illustrations and after.
